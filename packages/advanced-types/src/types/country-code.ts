import { CustomType, is, decode, m } from '@mondrian-framework/model'
  ...
  return m.custom(
    {
      ...
      is(input) {
        ...
        if (!COUNTRY_CODE_REGEX.test(<string>input)) {
          return {
            success: false,
            errors: [{ error: 'Invalid country code [ISO 3166-1 alpha-2]', value: input }],
          }
        }
        return { success: true }
      },
