  ...
  functions: {
    split: functions
      .define({
        input: model.object({
          str: model.string(),
          separator: model.string({ maxLength: 1, minLength: 1 }),
          max: model.object({ value: model.number().optional() }),
        }),
        output: model.string().array(),
      })
      .implement({
        async body({ input: { separator, str } }) {
          return result.ok(str.split(separator))
        },
      }),
    sum: functions
      .define({
        input: model.number().setName('n'),
        output: model.number(),
      })
      .implement({
        async body({ input }) {
...
const p = buildProgram({
  api: build({
    ...
    functions: {
      split: true,
      sum: { command: 'add', inputBindingStyle: 'argument-spreaded' },
    },
  }),
  context: async () => {},
})
